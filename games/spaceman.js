...
const Spaceman = (() => {
  ...
  function _updatePos(W, H) {
    if (_phase === 'ready') {
      ...
      /* Simpan posisi crash sekali */
      if (_crashStartY === null) { _crashStartY = _ay; _crashStartX = _ax; }
      /* Turun pelan sambil pegang balon — pakai easeIn supaya natural */
      const fallProg = Math.min(1, _crashT * 0.22);
      const fallE    = fallProg * fallProg;   /* easeIn: mulai lambat */
      _tx = _crashStartX + Math.sin(_balloonT * 0.3) * 8;   /* goyang pelan */
      _ty = _crashStartY + fallE * (H * 0.82 - _crashStartY);
      _ax += (_tx - _ax) * 0.018;
      _ay += (_ty - _ay) * 0.018;
      _tilt = Math.sin(_balloonT * 0.4) * 0.10;
    }
  }
  /* ─────────────────────────────────
  ...
  function _drawAstronaut(ctx, W, H) {
    ...
    if (_phase === 'ready') {
      ...
      ctx.beginPath(); ctx.moveTo(-4, -4); ctx.lineTo(-6, -22); ctx.stroke();

      ctx.scale(1.0, 1.0);
      _drawBody(ctx, 'crashed');
  ...
  function _drawBody(ctx, pose) {
    /* Kaki */
    ctx.fillStyle = '#3344bb';
    if (pose === 'ready') {
      /* Kaki tegak berdampingan */
      ctx.beginPath(); ctx.roundRect(-6, 14, 5, 10, 2); ctx.fill();
      ctx.beginPath(); ctx.roundRect(1, 14, 5, 10, 2); ctx.fill();
    } else {
    ...
    /* Tangan — beda per pose */
    ctx.fillStyle = '#e2e2e8';
    if (pose === 'ready') {
      /* Tangan lurus ke bawah (🧍) */
      ctx.beginPath(); ctx.roundRect(8,  4, 4, 8, 2); ctx.fill();
      ctx.beginPath(); ctx.roundRect(-12, 4, 4, 8, 2); ctx.fill();
    } else if (pose === 'flying') {
      /* Tangan kanan maju ke depan (🦸) */
      ctx.beginPath(); ctx.roundRect(8, 2, 5, 4, 2); ctx.fill();   /* kanan maju */
      ctx.beginPath(); ctx.roundRect(-13, 6, 5, 4, 2); ctx.fill(); /* kiri ke belakang */
    } else {
      /* Default */
